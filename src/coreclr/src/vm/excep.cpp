...
void GetCurrentExceptionPointers(PEXCEPTION_POINTERS pExceptionInfo DEBUG_ARG(bool checkExceptionRecordLocation))
{
    ...
    pExceptionInfo->ContextRecord = (PCONTEXT)ClrFlsGetValue(TlsIdx_PCONTEXT);
    pExceptionInfo->ExceptionRecord = (PEXCEPTION_RECORD)ClrFlsGetValue(TlsIdx_PEXCEPTION_RECORD);
...
DWORD GetCurrentExceptionCode()
{
    ...
    return (DWORD)ClrFlsGetValue(TlsIdx_EXCEPTION_CODE);
}
...
void SaveCurrentExceptionInfo(PEXCEPTION_RECORD pRecord, PCONTEXT pContext)
{
    ...
    if (CheckThreadStateNoCreate(TlsIdx_PEXCEPTION_RECORD))
    {
        BOOL fSave = TRUE;
